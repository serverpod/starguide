...
''';
    ...
    test(
      'when inlining partials then components are replaced with content',
      () async {
    ...
    test(
      'when a partial cannot be loaded then the page is left untouched',
      () async {
  ...
  group('Given nested partials', () {
    test(
      'when inlining then nested imports resolve relative to the partial',
      () async {
        final requested = <String>[];
        final partials = {
          'partials/_outer.md':
              "import Inner from './_inner.md';\n\n<Inner/>\n",
